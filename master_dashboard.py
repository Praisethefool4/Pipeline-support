...
<body>
    ...
    <div class="container-fluid px-4">
        ...
        <div class="row row-cols-1 row-cols-md-2 row-cols-lg-4 g-3" id="workers-grid">
            <!-- Dynamic Injection -->
        </div>
    ...
    <script>
        async function fetchState() {
            try {
                ...
                // Update Global Stats
                document.getElementById('stat-active-workers').innerText = `${data.active_count} / 20`;
                ...
                Object.keys(data.workers).sort((a,b) => parseInt(a) - parseInt(b)).forEach(id => {
                    ...
                    // Check if the worker is blocked or rate-limited
                    const isBlocked = wState.error_details && (
                        wState.error_details.includes('429') || 
                        wState.error_details.toLowerCase().includes('rate') ||
                        wState.error_details.toLowerCase().includes('block')
                    );
                    ...
                    if (isBlocked) {
                        ...
                        activeSpeed += 1.5; // Estimated crawl speed per running worker
                    ...
                    const borderStyle = isBlocked ? 'border: 2px solid #dc2626;' : '';
                    
                    const cardHtml = `
                        <div class="col">
                            <div class="card p-3" style="${borderStyle}">
                    ...
                    if (wState.logs && wState.logs.length > 0) {
                        wState.logs.slice(-3).forEach(l => {
                            logs.push(`[Worker #${id}] [${l.time || 'N/A'}] Q: ${l.query || 'None'} P: ${l.page || 0} -> Found: ${l.urls || 0} [${l.status || 'OK'}]`);
                        });
                    }
                });
                
                // Update active harvesting speed
                document.getElementById('stat-harvest-speed').innerText = `${Math.round(activeSpeed)} dorks/m`;
        ...
        async function globalAction(action) {
            if (!confirm(`Are you sure you want to trigger "${action}" on all 20 workers?`)) return;
            try {
...
def register():
    ...
    # Auto-start the worker immediately upon registration
    try:
        requests.post(f"{worker_url}/start", json={}, timeout=2)
        print(f"[Master] Auto-started Worker #{worker_id} on registration.")
    except Exception as e:
        print(f"[Master] Failed to auto-start Worker #{worker_id}: {e}")
